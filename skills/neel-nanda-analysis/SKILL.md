...
description: >-
  ...
  something rather than a neutral summary, in any language: "review this",
  "what do you make of X", "is this claim convincing", "poke holes in this",
  "give me your honest take", or when the user is reacting to a paper, RFC,
  strategy memo, or technical result and wants to know what to believe and how
  strongly. Prefer this over a plain summary any time the task is judgment, not
  description, even if the user doesn't name Neel Nanda.
---
...
and reserve the whole structure for something you're genuinely trying to
evaluate.
...
- **Empirical / scientific claim**: a fact about the world the work asserts
  ("this specific set of components implements behavior B").
- **Methodological claim**: the technique reliably does what it says.
- **Pragmatic claim**: it's actually useful in practice.
- **Interpretive / framing claim**: the bigger story or analogy the work hangs
  on ("this shows the system runs a human-legible algorithm").
...
them and aim your scrutiny at the ambitious one. For example, "the system
computes these quantities somewhere" is cheap and has a high prior; "those
quantities live in one privileged, selective mechanism" is the load-bearing
novelty. Finding that fault line, the point where a safe claim quietly becomes a
strong one, is often the single most valuable thing the decomposition does, and
it's where the work's own framing will most tempt you to grant the strong
version for free.

### 2. Rate each claim on the three axes

For every claim, say explicitly how important it is, how much you believe it,
and whether you can judge it. Lead with the split:
...
> Take a task like "When John and Mary went to the store, John gave a drink to
> ___" → "Mary". To get this right the system has to notice the repeated name,
> suppress it, and copy the other one. If you trace where that name-information
> moves, a small number of components turn out to be doing identifiable jobs,
> which is exactly the kind of structure the claim says is there.
...
> Ablating one component drops task accuracy. Tempting to call it *the*
> mechanism. But the boring version: ablating almost *any* capable component
> degrades performance broadly, so the drop may reflect general capability loss,
> not a task-specific role. What makes this non-boring is [the specific result
> the boring version can't produce, e.g. the ablation flipping the output to a
> specific wrong answer, not just degrading it].
...
visualization is only suggestive; the correlational probe I trust least."
Uniform enthusiasm is the tell of a shallow read.
...
> I'd expect this to be useful for *explaining one specific behavior you already
> have in hand*: run it when you have a narrow thing to account for. I would
> *not* expect the result to be the whole story: expect redundant mechanisms you
> didn't map, and expect it to transfer poorly to superficially similar cases.
...
stuck on how to phrase a judgment (more variants in
...
- "I expect this to help *find* candidates, not to *certify* their absence."
...
reasoning. Mixing them into non-English prose is expected and correct;
translating them dilutes the meaning. If a term would be opaque to the reader,
gloss it once in the local language, then use the English term thereafter.

One more style note: avoid leaning on em-dashes. Prefer commas, colons,
semicolons, parentheses, or a new sentence. The reasoning should carry the
rhythm, not the punctuation.
